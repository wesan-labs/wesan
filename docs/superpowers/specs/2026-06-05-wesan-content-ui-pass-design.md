...
- `en.json` - safe content fixes:
  ...
  - newsroom `wesan-announces-itself` lede: removed the claim "Wesan A.Ş. is registered in İstanbul. Two desks…" which **contradicts the post's own body** ("one person, one desk in Belgium, registration to follow") and the investors page ("no formal entity registration yet"). Now: "Wesan starts in Belgium - one desk, three divisions, one mission. Entity registration to follow. Today is day one."
...
## Not doing (YAGNI)
Mass refactor of all 26 routes; rewriting Can's literary voice unilaterally; the other 4 table pages until their slice comes up.
